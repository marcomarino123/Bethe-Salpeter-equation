...
cx_mat function_building_exponential_factor(int htb_basis_dimension,field<mat> wannier_centers,int number_g_points_list,mat g_points_list,int number_k_points_list,int spinorial_calculation,vec excitonic_momentum,int optical_limit){
			...
			for(int spin_channel=0;spin_channel<2;spin_channel++)
				for(int g=0; g<number_g_points_list; g++){
					for(int i=0; i<htb_basis_dimension_2; i++){
						exponential_factor(spin_channel*htb_basis_dimension_2+i,g).real(cos(accu((wannier_centers(spin_channel)).col(i)%(g_points_list.col(g)+excitonic_momentum))));
						exponential_factor(spin_channel*htb_basis_dimension_2+i,g).imag(sin(accu((wannier_centers(spin_channel)).col(i)%(g_points_list.col(g)+excitonic_momentum))));
					}
				}
		}else{
			for(int g=0; g<number_g_points_list; g++)
				for(int i=0; i<htb_basis_dimension; i++){
					exponential_factor(i,g).real(cos(accu(wannier_centers(0).col(i)%(g_points_list.col(g)+excitonic_momentum))));
					exponential_factor(i,g).imag(sin(accu(wannier_centers(0).col(i)%(g_points_list.col(g)+excitonic_momentum))));
				}
		}
	}else{
		if(spinorial_calculation==1){
			for(int spin_channel=0;spin_channel<2;spin_channel++)
				for(int g=0; g<number_g_points_list; g++){
					for(int i=0; i<htb_basis_dimension_2; i++){
						exponential_factor(spin_channel*htb_basis_dimension_2+i,g).real(-sin(accu( wannier_centers(spin_channel).col(i)%(g_points_list.col(g)) ) ) * accu( wannier_centers(spin_channel).col(i)%(excitonic_momentum) ) );
						exponential_factor(spin_channel*htb_basis_dimension_2+i,g).imag(cos(accu( wannier_centers(spin_channel).col(i)%(g_points_list.col(g)) ) ) * accu( wannier_centers(spin_channel).col(i)%(excitonic_momentum) ) );
					}
				}
		}else{
			for(int g=0; g<number_g_points_list; g++)
...
void K_points::push_k_points_list_values_fbz(double spacing_fbz_k_points_list)
{
	...
	vec bz_number_k_points_list(3);
	for (int i = 0; i < 3; i++)
		bz_number_k_points_list(i) = int(sqrt(accu(primitive_vectors.col(i) % primitive_vectors.col(i))) / spacing_fbz_k_points_list);

	int limiti = int(bz_number_k_points_list(0));
	int limitj = int(bz_number_k_points_list(1));
	int limitk = int(bz_number_k_points_list(2));
...
public:
	...
	void print_profile(mat k_points_path, int number_k_points_list){
		for(int i=0;i<number_k_points_list;i++)
...
public:
	...
	void push_values(int number_k_points_list_tmp, mat k_points_list_tmp, int number_g_points_list_tmp, mat g_points_list_tmp, int number_wannier_centers_tmp, int number_valence_bands_tmp, int number_conduction_bands_tmp, Hamiltonian_TB *hamiltonian_tb_tmp, int spinorial_calculation_tmp, int optical_limit_tmp){
...
tuple<mat,cx_mat> Dipole_Elements:: pull_values(vec excitonic_momentum){
	exponential_factor=function_building_exponential_factor(htb_basis_dimension, wannier_centers, number_g_points_list, g_points_list, number_k_points_list, spinorial_calculation, excitonic_momentum, optical_limit);
	...
	if(spinorial_calculation==1){
		...
		#pragma omp parallel for private(ks_states_k_point,ks_states_k_point_q,ks_state,ks_energy)
		for(int i=0;i<number_k_points_list;i++){
			//cout<<"k point: "<<i<<endl;
...
public:
	...
	cx_vec pull_susceptability(cx_vec omegas_path, int number_omegas_path, double eta);
};
cx_mat Dielectric_Function::pull_values(vec excitonic_momentum, cx_double omega, double eta){
	...
	if(spinorial_calculation==1){
		...
		#pragma omp parallel for private(energy,position)
		for(int i=0;i<number_k_points_list;i++)
			for(int spin_channel=0;spin_channel<2;spin_channel++)
		...
		#pragma omp parallel for private(excitonic_momentum_g,coulomb,rho_reduced_single_column_modified)
		for(int i=0;i<number_g_points_list;i++){
...
cx_vec Dielectric_Function::pull_susceptability(cx_vec omegas_path,int number_omegas_path,double eta){
	cx_mat dielectric_susceptability;
	cx_mat dielectric_susceptability_along_omegas_path;
	dielectric_susceptability_along_omegas_path.zeros(number_omegas_path);
	/// here the optical limit is supposed in the dipole building
	vec k_point_0; k_point_0.zeros(3);
	...
	dielectric_susceptability_along_omegas_path=dielectric_susceptability_along_omegas_path/3;
...
public:
	...
	tuple<cx_mat,cx_mat> pull_excitonic_hamiltonian_and_dipoles(vec excitonic_momentum,double epsilon,double eta);
	tuple<cx_mat,cx_mat> separating_spin_channels(cx_mat excitonic_hamiltonian);
	...
	void pull_dielectric_tensor_bse(double eta, double epsilon, ofstream *file_diel, double energy_step, double max_energy);
	void print(vec excitonic_momentum, double epsilon, double eta)
	{		
		cout<<"BSE hamiltoian..."<<endl;
		tuple<cx_mat,cx_mat> hamiltonian_and_rho=pull_excitonic_hamiltonian_and_dipoles(excitonic_momentum, epsilon, eta);
...
tuple<cx_mat,cx_mat> Excitonic_Hamiltonian::pull_excitonic_hamiltonian_and_dipoles(vec excitonic_momentum, double epsilon, double eta)
{
	...
	cx_mat rho_0_tmp(spin_number_valence_plus_conduction*number_valence_plus_conduction*number_k_points_list,number_g_points_list, fill::zeros);
	...
	cout<<"dimension: "<<size(rho_0_tmp)<<endl;
	cout<<" varie: "<<number_conduction_bands<<" "<<number_valence_bands<<" "<<number_k_points_list<<endl;
	for(int i=0;i<3;i++){
		excitonic_momentum(i)+=minval;
		energies_rho_0=dipole_elements->pull_values(excitonic_momentum_0);
		rho_0_tmp=get<1>(energies_rho_0);
		cout<<"dimension: "<<size(rho_0)<<endl;
		...
		for(int g=0;g<number_g_points_list;g++)
			for(int s=0;s<spin_number_valence_plus_conduction*number_valence_plus_conduction*number_k_points_list;s++)
				rho_0(s,g)=rho_0(s,g)+(rho_0_tmp(s,g))/3.0;
	...
	cx_mat w_matrix;
	w_matrix.zeros(spin_dimension_bse_hamiltonian,spin_dimension_bse_hamiltonian);
	cx_mat v_matrix;
	v_matrix.zeros(spin_dimension_bse_hamiltonian,spin_dimension_bse_hamiltonian);
	...
	if (spinorial_calculation == 1)
	{
		...
		//#pragma omp parallel for private(row,column,spin_v1,spin_c1,spin_v2,spin_c2,temporary_matrix1,temporary_matrix2)
		//for(int i=0;i<number_k_points_list;i++)
		...
		//								temporary_matrix1=(v_coulomb_qg)*(rho_cc.row(spin_c1*number_conduction_bands*number_conduction_bands*number_k_points_list+c1*number_conduction_bands*number_k_points_list+c2*number_k_points_list+i).t());
		...
		//								temporary_matrix2=v_coloumb_g%((rho_cv_0.row(spin_c1*number_conduction_bands*number_valence_bands*number_k_points_list+c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+i)).t());
		//								v_matrix(row,column)=accu(temporary_matrix2%((rho_cv_0.row(spin_c2*number_conduction_bands*number_valence_bands*number_k_points_list+c2*number_valence_bands*number_k_points_list+v2*number_k_points_list+j)).t()));
		//							}
		//			}
			...
			for(int j=0;j<2;j++){
				if(i==j)
					v_matrix.submat(i*3*number_conduction_bands*number_valence_bands*number_k_points_list,j*3*number_conduction_bands*number_valence_bands*number_k_points_list,(i*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1,(j*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)=
						...
						*(temporary_matrix1.submat(i*number_conduction_bands*number_valence_bands*number_k_points_list,0,(i+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1,number_g_points_list-1)).t();
				else
					v_matrix.submat(i*3*number_conduction_bands*number_valence_bands*number_k_points_list,j*3*number_conduction_bands*number_valence_bands*number_k_points_list,(i*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1,(j*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)=
						conj(rho_cv_0.submat(i*number_conduction_bands*number_valence_bands*number_k_points_list,0,(1+i)*number_valence_bands*number_valence_bands*number_k_points_list-1,number_g_points_list-1))
		...
		/////calculating w
		uvec rows_indices; rows_indices.set_size(number_conduction_bands);
		for(int c1=0;c1<number_conduction_bands;c1++)
		...
		int spinv2; int spinc2;
		#pragma omp parallel for private(temporary_matrix4,spinv2,spinc2)
				...
				for(int c2=0;c2<number_conduction_bands;c2++)
					for(int v2=0;v2<number_valence_bands;v2++){
						...
						w_matrix.submat(spinc2*2*number_conduction_bands*number_valence_bands*number_k_points_list+
							...
							spin2*number_conduction_bands*number_valence_bands*number_k_points_list,
							spinc2*2*number_conduction_bands*number_valence_bands*number_k_points_list+
							spinv2*number_conduction_bands*number_valence_bands*number_k_points_list+c2*number_valence_bands*number_k_points_list+v2*number_k_points_list+k2,
							(spin2+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)=
							vectorise(temporary_matrix4.rows(rows_indices),1);
					}
			}
		}
		cout<<"end calculation coupling elements"<<endl;
		excitonic_hamiltonian=w_matrix+v_matrix;
		...
		//				excitonic_hamiltonian(row,row)+=energies_0(spin_v1,v1*number_conduction_bands*number_k_points_list+c1*number_k_points_list+i);
		//			}
		for(int i=0;i<2;i++)
			excitonic_hamiltonian.submat(i*3*number_conduction_bands*number_valence_bands*number_k_points_list,i*3*number_conduction_bands*number_valence_bands*number_k_points_list,(i*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1,(i*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)=
				excitonic_hamiltonian.submat(i*3*number_conduction_bands*number_valence_bands*number_k_points_list,i*3*number_conduction_bands*number_valence_bands*number_k_points_list,(i*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1,(i*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)+diagmat(energies_0.row(i));
	}
	else
	{
		//int row; int column;
		...
		//							temporary_matrix1=(v_coulomb_qg)*(rho_cc.row(c1*number_conduction_bands*number_k_points_list+c2*number_k_points_list+i).t());
		...
		//							temporary_matrix2=(v_coulomb_g%(rho_cv_0.row(c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+i).t()));
		//							v_matrix(row,column)=accu(temporary_matrix2%(rho_cv_0.row(c2*number_valence_bands*number_k_points_list+v2*number_k_points_list+i).t()));
		//						}
		...
		//			excitonic_hamiltonian(row,row)+=energies_0(0,v1*number_conduction_bands*number_k_points_list+c1*number_k_points_list+i);
		//		}
	...
	return {excitonic_hamiltonian,rho_cv_0};
};
...
tuple<cx_mat,cx_mat> Excitonic_Hamiltonian:: separating_spin_channels(cx_mat excitonic_hamiltonian)
{
	...
	excitonic_hamiltonian_0.set_size(dimension_bse_hamiltonian_2,dimension_bse_hamiltonian_2);
	excitonic_hamiltonian_1.set_size(dimension_bse_hamiltonian_2,dimension_bse_hamiltonian_2);
	...
	transformation_matrix(0,3)=1.0; transformation_matrix(3,2)=1.0;
	transformation_matrix(1,0)=1.0/sqrt(2); transformation_matrix(1,1)=1.0/sqrt(2); transformation_matrix(1,3)=1.0/sqrt(2);
	transformation_matrix(2,0)=-1.0/sqrt(2); transformation_matrix(2,1)=-1.0/sqrt(2); transformation_matrix(2,3)=1.0/sqrt(2);
	...
	excitonic_hamiltonian_0=transformed_excitonic_hamiltonian.submat(0,0,dimension_bse_hamiltonian_2-1,dimension_bse_hamiltonian_2-1);
	excitonic_hamiltonian_1=transformed_excitonic_hamiltonian.submat(dimension_bse_hamiltonian_2,dimension_bse_hamiltonian_2,dimension_bse_hamiltonian_4-1,dimension_bse_hamiltonian_4-1);
	return{excitonic_hamiltonian_0,excitonic_hamiltonian_1};
};
...
tuple<cx_vec,cx_mat> Excitonic_Hamiltonian::pull_eigenstates_through_usualway(cx_mat excitonic_hamiltonian)
{
	...
	if (spinorial_calculation == 1)
	{
		tuple<cx_mat,cx_mat> excitonic_hamiltonian_01 =separating_spin_channels(excitonic_hamiltonian);
		cx_mat excitonic_hamiltonian_0 = get<0>(excitonic_hamiltonian_01);
		cx_mat excitonic_hamiltonian_1 = get<1>(excitonic_hamiltonian_01);
		///diagonalizing the two spin channels separately: M=0 and M=\pm1
		cx_vec eigenvalues_1; cx_mat eigenvectors_1;
		cx_vec eigenvalues_0; cx_mat eigenvectors_0;
		...
		cx_vec exc_eigenvalues_1; cx_vec exc_eigenvalues_0;
		...
		for (int i=0;i<dimension_bse_hamiltonian_2;i++)
		{
			...
			for(int s=0;s<dimension_bse_hamiltonian;s++)
				exc_eigenvectors(s+dimension_bse_hamiltonian_2,i+dimension_bse_hamiltonian_2)=eigenvectors_0(s,ordering_0(i))/modulus; 
			exc_eigenvalues(i+dimension_bse_hamiltonian_2) = exc_eigenvalues_0(i);
		}
...
tuple<cx_mat,cx_mat> Excitonic_Hamiltonian:: pull_excitonic_resonant_and_coupling_part(vec excitonic_momentum, double epsilon, double eta){
	...
	cx_mat r_matrix;
	r_matrix.zeros(spin_dimension_bse_hamiltonian,spin_dimension_bse_hamiltonian);
	cx_mat c_matrix;
	c_matrix.zeros(spin_dimension_bse_hamiltonian,spin_dimension_bse_hamiltonian);
	if (spinorial_calculation == 1)
	{
		//int spin_c1; int spin_v1; int spin_c2; int spin_v2;
		//int row; int column;
		//cx_vec temporary_matrix1(number_g_points_list);
		//cx_vec temporary_matrix2(number_g_points_list);
		//cx_double part_1; cx_double part_2;
		//#pragma omp parallel for private(row,column,spin_v1,spin_c1,spin_v2,spin_c2,temporary_matrix1,temporary_matrix2,part_1,part_2)
		//for(int i=0;i<number_k_points_list;i++)
		//	for(int v1=0;v1<number_valence_bands;v1++)
		//		for(int c1=0;c1<number_conduction_bands;c1++)
		//			for(int spin_channel1=0;spin_channel1<4;spin_channel1++){
		...
		//								column=spin_v2*2*number_conduction_bands*number_valence_bands*number_k_points_list+spin_c2*number_conduction_bands*number_valence_bands*number_k_points_list+v2*number_conduction_bands*number_k_points_list+c2*number_k_points_list+j;
		//								/// resonant part
		//								temporary_matrix1=v_coulomb_unscreened%(rho_cv.row(spin_c2*number_conduction_bands*number_valence_bands*number_k_points_list+c2*number_valence_bands*number_k_points_list+v2*number_k_points_list+j).t());
		//								part_1=accu((temporary_matrix1.t())%conj(rho_cv.row(spin_c1*number_conduction_bands*number_valence_bands*number_k_points_list+c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+i)));				
		//								temporary_matrix2=v_coulomb_screened*(rho_cv.row(spin_c2*number_conduction_bands*number_valence_bands*number_k_points_list+c2*number_valence_bands*number_k_points_list+v2*number_k_points_list+j).t());
		//								part_2=accu((temporary_matrix1.t())%conj(rho_cv.row(spin_c1*number_conduction_bands*number_valence_bands*number_k_points_list+c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+i)));				
		//								resonant_part(row,column)=part_1+part_2;
		//								/// coupling part
		//								temporary_matrix1=v_coulomb_unscreened%(rho_vc.row(spin_v2*number_conduction_bands*number_valence_bands*number_k_points_list+v2*number_conduction_bands*number_k_points_list+c2*number_k_points_list+j).t());
		//								part_1=accu((temporary_matrix1.t())%conj(rho_cv.row(spin_c1*number_conduction_bands*number_valence_bands*number_k_points_list+c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+i)));				
		//								temporary_matrix2=v_coulomb_screened*(rho_vc.row(spin_v2*number_conduction_bands*number_valence_bands*number_k_points_list+v2*number_conduction_bands*number_k_points_list+c2*number_k_points_list+j).t());
		//								part_2=accu((temporary_matrix1.t())%conj(rho_cv.row(spin_c1*number_conduction_bands*number_valence_bands*number_k_points_list+c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+i)));				
		//								coupling_part(row,column)=part_1+part_2;
		//							}
		//			}
		...
		r_matrix=r_matrix/coulomb_potential->pull_cell_volume();
		c_matrix=c_matrix/coulomb_potential->pull_cell_volume();
...
tuple<vec,mat> Excitonic_Hamiltonian:: pull_eigenstates_through_cholesky_way(vec excitonic_momentum, double epsilon, double eta)
{
	...
	mat M; M.set_size(spin_dimension_bse_hamiltonian_2, spin_dimension_bse_hamiltonian_2);
	...
	///}
	/// construct W
	mat J; J.zeros(spin_dimension_bse_hamiltonian_2,spin_dimension_bse_hamiltonian_2);
	for (int q = 0; q < spin_dimension_bse_hamiltonian_2; q++)
		for (int s = 0; s < spin_dimension_bse_hamiltonian_2; s++)
		{
	...
	vec exc_eigenvalues;
	exc_eigenvalues.set_size(spin_dimension_bse_hamiltonian_2);
	exc_eigenvalues = sort(eigenvalues);
	/// normalizing and ordering eigenvectors
	...
	mat exc_eigenvectors;
	exc_eigenvectors.set_size(spin_dimension_bse_hamiltonian_2,spin_dimension_bse_hamiltonian_2);
...
cx_vec Excitonic_Hamiltonian::pull_excitonic_oscillator_force(cx_mat excitonic_eigenstates, cx_mat rho_cv_0){
	...
	if(spinorial_calculation==1){
		cx_mat excitonic_eigenstates_reduced=excitonic_eigenstates.submat(0,0,dimension_bse_hamiltonian*2-1,dimension_bse_hamiltonian*2-1);
		oscillator_force.zeros(dimension_bse_hamiltonian*2);
		for(int i=0;i<dimension_bse_hamiltonian*2;i++)
			oscillator_force(i)=accu(conj(rho_cv_0.col(0))%excitonic_eigenstates_reduced.col(i));
	}else{
...
void Excitonic_Hamiltonian:: pull_dielectric_tensor_bse(double eta, double epsilon, ofstream *file_diel, double energy_step, double max_energy)
{
	cout << "Calculation of the dielectric tensor..." << endl;
	double factor=(pow(const_electron_charge, 2)/(const_vacuum_dielectric_constant*coulomb_potential->pull_cell_volume()*number_k_points_list));
	tuple<cx_mat,cx_mat> exc_hamiltonian_and_rho_cv_0;
	...
	cx_vec omega(number_of_energy_steps,fill::zeros);
	for(int i=0;i<number_of_energy_steps;i++)
		omega(i).real(double(i)*max_energy/double(number_of_energy_steps-1));

	cx_vec temporary_variable(number_of_energy_steps);

	cx_cube dielectric_tensor_bse(3,3,number_of_energy_steps,fill::zeros);

	cout<<"dimension: "<<size(dielectric_tensor_bse)<<endl;
	vec excitonic_momentum1(3);
	vec excitonic_momentum2(3);
	cx_vec exc_oscillator_force1(dimension_bse_hamiltonian*2, fill::zeros);
	cx_vec exc_oscillator_force2(dimension_bse_hamiltonian*2, fill::zeros);
	for (int i = 0; i < 3; i++){
		for (int j = 0; j < 3; j++){
			if(i!=j){
				//excitonic_momentum1.zeros(3);
				///excitonic_momentum2.zeros(3);
				...
				exc_oscillator_force1=pull_excitonic_oscillator_force(exc_eigenstates,rho_cv_0);
				for(int l=0;l<dimension_bse_hamiltonian*2;l++)
					exc_oscillator_force2(l)=exc_oscillator_force1(l);
				excitonic_momentum1(i)-=minval;
			}
			cout<<"dimension ex: "<<size(exc_oscillator_force1)<<endl;
			temporary_variable.zeros(number_of_energy_steps);
			/////#pragma omp parallel for private(exc_oscillator_force2,exc_oscillator_force1,omega,exc_eigenvalues,dielectric_tensor_bse)
			for(int s=0;s<number_of_energy_steps;s++){
				for(int l=0;l<dimension_bse_hamiltonian*2;l++)
					temporary_variable(s)=temporary_variable(s)+(exc_oscillator_force2(l))*conj(exc_oscillator_force1(l))/(omega(s)-exc_eigenvalues(l));
				dielectric_tensor_bse(i,j,s)=(factor/pow(minval,2))*temporary_variable(s);
			}
		}
	}
	///writing the dielectric function (in the optical limit) in a file
	*file_diel<<"omega xx xy xz yx yy yz zx zy zz"<<endl;
	for(int s=0;s<number_of_energy_steps;s++){
		for (int i = 0; i < 3; i++)
			for (int j = 0; j < 3; j++)
				*file_diel<<dielectric_tensor_bse(i,j,s)<<" ";
		*file_diel<<endl;
	}
};
...
int main()
{
	...
	double cutoff_g_points_list=10;
	...
	//int number_k_points_list_path=1000;
	//mat k_points_path; k_points_path.zeros(3,number_k_points_list_path);
	//for(int i=0;i<number_k_points_list_path;i++)
	//	k_points_path(0,i)=double(i);
	//coulomb_potential.print_profile(k_points_path,number_k_points_list_path);
	...
	int number_conduction_bands_bse=5;
	int number_valence_bands_bse=5;
	Dipole_Elements dipole_elements;
	vec excitonic_momentum; excitonic_momentum.zeros(3);
	int optical_limit=1;
	//dipole_elements.push_values(number_k_points_list,k_points_list,number_g_points_list,g_points_list,number_wannier_centers,number_valence_bands_bse,number_conduction_bands_bse,&htb,spinorial_calculation, optical_limit);
	...
	///int number_omegas_points=20;
	///double omega_max=6.00;
	///cx_vec omegas(number_omegas_points);
	//cout<<"susceptibility"<<endl;
	//for(int i=0;i<number_omegas_points;i++){
	//	omegas(i)=double(i)/double(number_omegas_points)*omega_max;
	//}
	//cx_vec susceptability=dielectric_function.pull_susceptability(omegas,number_omegas_points,eta);
	//for(int i=0;i<number_omegas_points;i++)
	...
	//double max_energy=6;
	//htbse.pull_dielectric_tensor_bse(eta,epsilon,&file_diel,energy_step,max_energy);
	//htbse.print(excitonic_momentum,epsilon,eta);
	//file_diel.close();
	//file_htb.close();
