...
const double conversionNmtoeV = 1.0;
...
public:
	...
	///rho_{n1,n2,k1-p,k2-q}(excitonic_momentum,G)=\bra{n1k1-p}e^{i(excitonic_momentum+G)r\ket{n2k2-q}
	tuple<cx_mat,cx_mat> pull_values(vec excitonic_momentum,vec p_parameter,vec q_parameter,int diagonal_k);
	cx_mat pull_reduced_values_vc(cx_mat rho,int diagonal_k,int rotated);
...
tuple<cx_mat,cx_mat> Dipole_Elements::pull_values(vec excitonic_momentum,vec p_parameter,vec q_parameter,int diagonal_k){
	...
	exponential_factor=function_building_exponential_factor(excitonic_momentum);
	...
	if(diagonal_k==1){
		...
		///for(int i=0;i<number_k_points_list;i++){
		///	ks_states_k_point = hamiltonian_tb->pull_ks_states_subset(k_points_list.col(i)-p_parameter,number_valence_bands,number_conduction_bands);
		///	ks_energy=get<0>(ks_states_k_point); ks_state=get<1>(ks_states_k_point);
		///	for(int spin_channel=0;spin_channel<(spinorial_calculation+1);spin_channel++)
		///		for(int j=0;j<number_k_points_list;j++)
		///			for(int m=0;m<number_valence_plus_conduction;m++){
		///				for(int g=0;g<number_g_points_list;g++)
		///					ks_state_left.subcube(spin_channel*spin_htb_basis_dimension,m*effective_number_k_points_list+i*number_k_points_list+j,g,(spin_channel+1)*spin_htb_basis_dimension-1,m*effective_number_k_points_list+i*number_k_points_list+j,g)=
		///						ks_state.submat(spin_channel*spin_htb_basis_dimension,m,(spin_channel+1)*spin_htb_basis_dimension-1,m);
		///				energies(spin_channel,m*effective_number_k_points_list+i*number_k_points_list+j).real(ks_energy(spin_channel,m));
		///			}
		///}
			...
			for(int m=0;m<number_valence_plus_conduction;m++)
				for(int g=0;g<number_g_points_list;g++)
					for(int j=0;j<number_k_points_list;j++){
						...
						for(int i=0;i<number_k_points_list;i++){
							ks_state_right.subcube(spin_channel*spin_htb_basis_dimension,m*effective_number_k_points_list+i*number_k_points_list+j,g,(spin_channel+1)*spin_htb_basis_dimension-1,m*effective_number_k_points_list+i*number_k_points_list+j,g)=
								exponential_factor.submat(spin_channel*spin_htb_basis_dimension,g,(spin_channel+1)*spin_htb_basis_dimension-1,g)%ks_state_q.submat(spin_channel*spin_htb_basis_dimension,m,(spin_channel+1)*spin_htb_basis_dimension-1,m);
							ks_state_left.subcube(spin_channel*spin_htb_basis_dimension,m*effective_number_k_points_list+i*number_k_points_list+j,g,(spin_channel+1)*spin_htb_basis_dimension-1,m*effective_number_k_points_list+i*number_k_points_list+j,g)=
			...
			for(int m=0;m<number_valence_plus_conduction;m++)
				for(int n=0;n<number_valence_plus_conduction;n++)
					rho.submat(spin_channel*number_valence_plus_conduction*number_valence_plus_conduction*effective_number_k_points_list+m*number_valence_plus_conduction*effective_number_k_points_list+n*effective_number_k_points_list,0,spin_channel*number_valence_plus_conduction*number_valence_plus_conduction*effective_number_k_points_list+m*number_valence_plus_conduction*effective_number_k_points_list+n*effective_number_k_points_list+effective_number_k_points_list-1,number_g_points_list-1)=
						(cx_mat)(sum(conj(ks_state_left.subcube(spin_channel*spin_htb_basis_dimension,m*effective_number_k_points_list,0,(spin_channel+1)*spin_htb_basis_dimension-1,(m+1)*effective_number_k_points_list-1,number_g_points_list-1))%
...
cx_mat Dipole_Elements::pull_reduced_values_vc(cx_mat rho, int diagonal_k, int rotated){
	int effective_number_k_points_list;
	if(diagonal_k==0)
		effective_number_k_points_list=number_k_points_list*number_k_points_list;
	else
...
void Dipole_Elements::print(vec excitonic_momentum, vec p_parameter, int which_term, int diagonal_k){
	cx_mat exponential_factor(htb_basis_dimension,number_g_points_list);
	exponential_factor=function_building_exponential_factor(excitonic_momentum);
	...
	cx_mat dipole_elements_tot(spin_number_valence_plus_conduction*number_valence_plus_conduction*effective_number_k_points_list,number_g_points_list);
	tuple<cx_mat,cx_mat> energies_and_dipole_elements(energies,dipole_elements_tot);
	energies_and_dipole_elements=pull_values(excitonic_momentum,excitonic_momentum,p_parameter,diagonal_k);
	energies=get<0>(energies_and_dipole_elements);
	dipole_elements_tot=get<1>(energies_and_dipole_elements);
	int states1;
	if(which_term==0){
		...
		cx_mat dipole_elements_partial(states1,number_g_points_list);
		dipole_elements_partial=pull_reduced_values_vc(dipole_elements_tot,diagonal_k,0);
		for(int i=0;i<states1;i++){
			for(int g=0;g<number_g_points_list;g++)
		...
		cout<<states1<<endl;
		cx_mat dipole_elements_partial(states1,number_g_points_list);
		dipole_elements_partial=pull_reduced_values_cc_vv(dipole_elements_tot,0,0,diagonal_k);
		int count=0;
		...
		states1=spin_number_valence_bands*number_conduction_bands*effective_number_k_points_list;
		cx_mat dipole_elements_partial(states1,number_g_points_list);
		dipole_elements_partial=pull_reduced_values_cv(dipole_elements_tot,diagonal_k);
		for(int i=0;i<states1;i++){
			for(int g=0;g<number_g_points_list;g++)
				cout<<dipole_elements_partial(i,g)<<" ";
			cout<<endl;
		...
		cx_mat dipole_elements_partial(states1,number_g_points_list);
		dipole_elements_partial=pull_reduced_values_cc_vv(dipole_elements_tot,1,0,diagonal_k);
		for(int i=0;i<states1;i++){
			for(int g=0;g<number_g_points_list;g++)
				cout<<dipole_elements_partial(i,g)<<" ";
			cout<<endl;
...
cx_mat Dielectric_Function::pull_values(vec excitonic_momentum, cx_double omega, double eta){
	...
	cx_mat energies((spinorial_calculation+1),number_k_points_list*number_valence_plus_conduction*number_valence_plus_conduction);
	cx_mat rho((spinorial_calculation+1)*number_valence_plus_conduction*number_valence_plus_conduction*number_k_points_list,number_g_points_list);
	tuple<cx_mat,cx_mat> energies_rho(energies,rho);

	energies_rho=dipole_elements->pull_values(excitonic_momentum,zeros_vec,excitonic_momentum,1);
	cx_mat rho_cv((spinorial_calculation+1)*number_valence_bands*number_conduction_bands*number_k_points_list,number_g_points_list);
	rho_cv=dipole_elements->pull_reduced_values_cv(get<1>(energies_rho),1);
...
public:
	...
	Excitonic_Hamiltonian(int number_valence_bands_tmp,int number_conduction_bands_tmp, mat k_points_list_tmp, int number_k_points_list_tmp, mat g_points_list_tmp,int number_g_points_list_tmp, int spinorial_calculation_tmp, int htb_basis_dimension_tmp,Dipole_Elements *dipole_elements_tmp);
	void pull_coulomb_potentials(Coulomb_Potential* coulomb_potential,Dielectric_Function* dielectric_function,int adding_screening,vec excitonic_momentum,double eta);
	tuple<cx_mat,cx_mat> pull_resonant_part_and_rcv(vec excitonic_momentum,double eta);
	cx_mat add_coupling_part(cx_mat excitonic_hamiltonian,cx_mat rho_q_diagk_cv,vec excitonic_momentum,double eta);
	tuple<cx_vec,cx_mat,cx_mat> common_diagonalization(vec excitonic_momentum,double eta,Coulomb_Potential *coulomb_potential,Dielectric_Function *dielectric_function,int adding_screening,int tamn_dancoff);
	tuple<vec,cx_mat,cx_mat> cholesky_diagonalization(vec excitonic_momentum,double eta,Coulomb_Potential *coulomb_potential,Dielectric_Function *dielectric_function,int adding_screenin);
	cx_vec pull_excitonic_oscillator_force(cx_mat excitonic_eigenstates,cx_mat rho_cv);
	void pull_macroscopic_bse_dielectric_function(cx_vec omegas_path,int number_omegas_path,double eta,string file_macroscopic_dielectric_function_bse_name,double lorentzian,int tamn_dancoff,Coulomb_Potential* coulomb_potential,Dielectric_Function* dielectric_function,int adding_screening);
	void print(vec excitonic_momentum,double eta,int tamn_dancoff,Coulomb_Potential* coulomb_potential,Dielectric_Function* dielectric_function,int adding_screening);
};
Excitonic_Hamiltonian::Excitonic_Hamiltonian(int number_valence_bands_tmp,int number_conduction_bands_tmp, mat k_points_list_tmp, int number_k_points_list_tmp, mat g_points_list_tmp,int number_g_points_list_tmp,int spinorial_calculation_tmp,int htb_basis_dimension_tmp,Dipole_Elements *dipole_elements_tmp){
	spinorial_calculation = spinorial_calculation_tmp;
	number_k_points_list = number_k_points_list_tmp;
	number_conduction_bands = number_conduction_bands_tmp;
	number_valence_bands = number_valence_bands_tmp;
...
void Excitonic_Hamiltonian::pull_coulomb_potentials(Coulomb_Potential* coulomb_potential,Dielectric_Function* dielectric_function,int adding_screening,vec excitonic_momentum,double eta){
	...
	for (int k = 0; k < number_g_points_list; k++){
		for (int s = 0; s < number_g_points_list; s++)
			v_coulomb_gg(k,s) = epsilon_inv_static(k,s)*coulomb_potential->pull(excitonic_momentum+g_points_list.col(s));
		v_coulomb_g(k) = coulomb_potential->pull(excitonic_momentum+g_points_list.col(k));
	}
};
tuple<cx_mat,cx_mat> Excitonic_Hamiltonian::pull_resonant_part_and_rcv(vec excitonic_momentum,double eta){
	...
	vec zeros_vec(3,fill::zeros);
	cx_mat energies_q((spinorial_calculation+1),number_k_points_list*number_valence_bands*number_conduction_bands);
	cx_mat rho_q_diagk((spinorial_calculation+1)*number_valence_plus_conduction*number_valence_plus_conduction*number_k_points_list,number_g_points_list);
	tuple<cx_mat,cx_mat> energies_rho_q_diagk(energies_q,rho_q_diagk);
	energies_rho_q_diagk=dipole_elements->pull_values(excitonic_momentum,zeros_vec,excitonic_momentum,1);
	cx_mat rho_q_diagk_cv((spinorial_calculation+1)*number_conduction_bands*number_valence_bands*number_k_points_list,number_g_points_list);
	rho_q_diagk_cv=dipole_elements->pull_reduced_values_cv(get<1>(energies_rho_q_diagk),1);
	...
	////excluding G=0
	temporary_matrix1.col(0)=zeros_long_vec;
	cx_mat rho_q_diagk_cv_tmp((spinorial_calculation+1)*number_conduction_bands*number_valence_bands*number_k_points_list,number_g_points_list);
	rho_q_diagk_cv_tmp=rho_q_diagk_cv;
	rho_q_diagk_cv_tmp.col(0)=zeros_long_vec;
	...
	for(int i=0;i<(spinorial_calculation+1);i++)
		for(int j=0;j<(spinorial_calculation+1);j++)
			v_matrix.submat(i*3*number_conduction_bands*number_valence_bands*number_k_points_list,j*3*number_conduction_bands*number_valence_bands*number_k_points_list,(i*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1,(j*3+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)=
				conj(rho_q_diagk_cv_tmp.submat(i*number_conduction_bands*number_valence_bands*number_k_points_list,0,(1+i)*number_conduction_bands*number_valence_bands*number_k_points_list-1,number_g_points_list-1))
	...
	///cout<<v_matrix-conj(v_matrix).t()<<endl;
	///building w
		...
		for(int v=0;v<number_valence_bands;v++)
			for(int k=0;k<number_k_points_list;k++)
				for(int l=0;l<number_k_points_list;l++)
					diag_indices(c*number_valence_bands*number_k_points_list+v*number_k_points_list+k,l)=
						v*number_k_points_list*number_k_points_list*number_conduction_bands*number_k_points_list*number_k_points_list
						+l*number_k_points_list*number_conduction_bands*number_k_points_list*number_k_points_list
						+k*number_conduction_bands*number_k_points_list*number_k_points_list
						+c*number_k_points_list*number_k_points_list
						+l*number_k_points_list
						+k;

	cx_vec temporary_matrix2(number_valence_bands*number_conduction_bands*number_k_points_list*number_k_points_list);
	...
	///	#pragma omp parallel for private(temporary_matrix2,diag_indices,spinv1,spinc1,spin1)
		for(int c1=0;c1<number_conduction_bands;c1++)
			for(int v1=0;v1<number_valence_bands;v1++)
				for(int k1=0;k1<number_k_points_list;k1++){
					temporary_matrix2=vectorise(((rho_kk_cc.submat(spinc1*number_conduction_bands*number_conduction_bands*number_k_points_list*number_k_points_list+c1*number_conduction_bands*number_k_points_list*number_k_points_list,0,spinc1*number_conduction_bands*number_conduction_bands*number_k_points_list*number_k_points_list+(c1+1)*number_conduction_bands*number_k_points_list*number_k_points_list-1,number_g_points_list-1)*v_coulomb_gg)
						*conj(rho_qq_kk_vv.submat(spinv1*number_valence_bands*number_valence_bands*number_k_points_list*number_k_points_list+v1*number_valence_bands*number_k_points_list*number_k_points_list,0,spinv1*number_valence_bands*number_valence_bands*number_k_points_list*number_k_points_list+(v1+1)*number_valence_bands*number_k_points_list*number_k_points_list-1,number_g_points_list-1)).t()),0);
					w_matrix.submat(spin1*number_conduction_bands*number_valence_bands*number_k_points_list+c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+k1,
						...
						(spin1+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)=(temporary_matrix2.elem(diag_indices.col(k1))).t();
				}
	}
	///cout<<w_matrix<<endl;
	///cout<<(w_matrix-conj(w_matrix).t())<<endl;
	...
	cx_mat excitonic_hamiltonian(2*spin_dimension_bse_hamiltonian,2*spin_dimension_bse_hamiltonian,fill::zeros);
	...
	excitonic_hamiltonian.submat(0,0,spin_dimension_bse_hamiltonian-1,spin_dimension_bse_hamiltonian-1)=((spinorial_calculation-2)*v_matrix-w_matrix);
	excitonic_hamiltonian.submat(spin_dimension_bse_hamiltonian,spin_dimension_bse_hamiltonian,2*spin_dimension_bse_hamiltonian-1,2*spin_dimension_bse_hamiltonian-1)=-conj((spinorial_calculation-2)*v_matrix-w_matrix).t();
	...
	//cout<<excitonic_hamiltonian-conj(excitonic_hamiltonian).t()<<endl;

	return {excitonic_hamiltonian,rho_q_diagk_cv};
};
cx_mat Excitonic_Hamiltonian::add_coupling_part(cx_mat excitonic_hamiltonian,cx_mat rho_q_diagk_cv,vec excitonic_momentum,double eta){
	...
	vec zeros_vec(3,fill::zeros);
	cx_mat rho_p_diagk_vc((spinorial_calculation+1)*number_valence_bands*number_conduction_bands*number_k_points_list,number_g_points_list);
	rho_p_diagk_vc=dipole_elements->pull_reduced_values_vc(get<1>(dipole_elements->pull_values(excitonic_momentum,-excitonic_momentum,zeros_vec,1)),1,1);
	cx_vec zeros_long_vec((spinorial_calculation+1)*number_conduction_bands*number_valence_bands*number_k_points_list,fill::zeros);
	cx_mat temporary_matrix1((spinorial_calculation+1)*number_conduction_bands*number_valence_bands*number_k_points_list,number_g_points_list);
	for(int i=0;i<(spinorial_calculation+1)*number_conduction_bands*number_valence_bands*number_k_points_list;i++)
		temporary_matrix1.row(i)=rho_q_diagk_cv.row(i)%v_coulomb_g.t();
	////excluding G=0
	temporary_matrix1.col(0)=zeros_long_vec;
	rho_q_diagk_cv.col(0)=zeros_long_vec;
	for(int i=0;i<(spinorial_calculation+1);i++)
		for(int j=0;j<(spinorial_calculation+1);j++)
	...
	cx_mat rho_q_kk_cv((spinorial_calculation+1)*number_valence_bands*number_conduction_bands*number_k_points_list*number_k_points_list,number_g_points_list);
	cx_mat rho_p_kk_vc((spinorial_calculation+1)*number_valence_bands*number_conduction_bands*number_k_points_list*number_k_points_list,number_g_points_list);
	rho_p_kk_vc=dipole_elements->pull_reduced_values_vc(get<1>(dipole_elements->pull_values(excitonic_momentum,excitonic_momentum,zeros_vec,0)),0,0);
	rho_q_kk_cv=dipole_elements->pull_reduced_values_cv(get<1>(dipole_elements->pull_values(excitonic_momentum,zeros_vec,-excitonic_momentum,0)),0);
	...
	cx_vec temporary_matrix3(number_valence_bands*number_conduction_bands*number_k_points_list*number_k_points_list);
	int spin2; int spin1;
	int spinv1; int spinc1;
	for(int spin1=0;spin1<(3*spinorial_calculation+1);spin1++){
		spinv1=exciton_spin(0,spin1);
		...
		for(int c1=0;c1<number_conduction_bands;c1++)
			for(int v1=0;v1<number_valence_bands;v1++)
				for(int k1=0;k1<number_k_points_list;k1++){
					cout<<c1<<" "<<v1<<" "<<k1<<endl;
					temporary_matrix3=vectorise((conj(rho_p_kk_vc.submat(spinc1*number_conduction_bands*number_valence_bands*number_k_points_list*number_k_points_list+c1*number_valence_bands*number_k_points_list*number_k_points_list,0,spinc1*number_conduction_bands*number_valence_bands*number_k_points_list*number_k_points_list+(c1+1)*number_valence_bands*number_k_points_list*number_k_points_list-1,number_g_points_list-1))*v_coulomb_gg)
						*(rho_q_kk_cv.submat(spinv1*number_conduction_bands*number_valence_bands*number_k_points_list*number_k_points_list+v1*number_conduction_bands*number_k_points_list*number_k_points_list,0,spinv1*number_conduction_bands*number_valence_bands*number_k_points_list*number_k_points_list+(v1+1)*number_conduction_bands*number_k_points_list*number_k_points_list-1,number_g_points_list-1)).t(),0);
					w_matrix_c.submat(spin1*number_conduction_bands*number_valence_bands*number_k_points_list+c1*number_valence_bands*number_k_points_list+v1*number_k_points_list+k1,
						...
						(spin2+1)*number_conduction_bands*number_valence_bands*number_k_points_list-1)=(temporary_matrix3(diag_indices.col(k1))).t();				
				}
	}
	...
	v_matrix_c=v_matrix_c/(number_k_points_list);
...
tuple<cx_vec,cx_mat,cx_mat> Excitonic_Hamiltonian::common_diagonalization(vec excitonic_momentum,double eta,Coulomb_Potential *coulomb_potential,Dielectric_Function *dielectric_function,int adding_screening,int tamn_dancoff){
	
	int dimension_bse_hamiltonian_2=2*spin_dimension_bse_hamiltonian;
	pull_coulomb_potentials(coulomb_potential,dielectric_function,adding_screening,excitonic_momentum,eta);
	cx_mat excitonic_hamiltonian(dimension_bse_hamiltonian_2,dimension_bse_hamiltonian_2);
	cx_mat rho_cv((spinorial_calculation+1)*number_conduction_bands*number_valence_bands*number_k_points_list,number_g_points_list);
	tuple<cx_mat,cx_mat> excitonic_hamiltonian_and_rho_cv(excitonic_hamiltonian,rho_cv);
	excitonic_hamiltonian_and_rho_cv=pull_resonant_part_and_rcv(excitonic_momentum,eta);
	if(tamn_dancoff==0)
		excitonic_hamiltonian=add_coupling_part(excitonic_hamiltonian,rho_cv,excitonic_momentum,eta);
	excitonic_hamiltonian=get<0>(excitonic_hamiltonian_and_rho_cv);
	rho_cv=get<1>(excitonic_hamiltonian_and_rho_cv);
	
	//for(int i=0;i<dimension_bse_hamiltonian_2;i++){
	//	for(int j=0;j<dimension_bse_hamiltonian_2;j++)
	//		cout<<excitonic_hamiltonian(i,j)-conj(excitonic_hamiltonian(j,i))<<" ";
	//	cout<<endl;
	//}
	//for(int i=0;i<dimension_bse_hamiltonian_2;i++){
	//	for(int j=0;j<dimension_bse_hamiltonian_2;j++)
	...
	if (spinorial_calculation == 1){
		...
		///false=0,true=1
		cout<<"hermitian "<<excitonic_hamiltonian.is_hermitian()<<endl;
		...
		for(int i=0;i<dimension_bse_hamiltonian_2;i++)
			exc_eigenvectors.col(i)=exc_eigenvectors.col(i)/norm(exc_eigenvectors.col(i),2);
		free(u);
		return {exc_eigenvalues.subvec(dimension_bse_hamiltonian,dimension_bse_hamiltonian_2-1),exc_eigenvectors.submat(dimension_bse_hamiltonian,dimension_bse_hamiltonian,dimension_bse_hamiltonian_2-1,dimension_bse_hamiltonian_2-1),rho_cv};
		///return{ exc_eigenvalues,exc_eigenvectors,rho_cv};
	}
};
...
tuple<vec,cx_mat,cx_mat> Excitonic_Hamiltonian:: cholesky_diagonalization(vec excitonic_momentum,double eta,Coulomb_Potential *coulomb_potential,Dielectric_Function *dielectric_function,int adding_screening)
{
	pull_coulomb_potentials(coulomb_potential,dielectric_function,adding_screening,excitonic_momentum,eta);
	tuple<cx_mat,cx_mat> excitonic_hamiltonian_and_rho_cv=pull_resonant_part_and_rcv(excitonic_momentum,eta);
...
void Excitonic_Hamiltonian:: pull_macroscopic_bse_dielectric_function(cx_vec omegas_path,int number_omegas_path,double eta,string file_macroscopic_dielectric_function_bse_name,double lorentzian,int tamn_dancoff,Coulomb_Potential* coulomb_potential,Dielectric_Function* dielectric_function,int adding_screening)
{
	...
	tuple<cx_vec,cx_mat,cx_mat> eigenvalues_and_eigenstates_and_rho_cv(exc_eigenvalues,exc_eigenstates,rho_cv);
	...
	for (int i = 0; i < 3; i++)
		for (int j = 0; j < 3; j++)
			if(i==j&&i==0){
				...
				eigenvalues_and_eigenstates_and_rho_cv = common_diagonalization(excitonic_momentum1,eta,coulomb_potential,dielectric_function,1,tamn_dancoff);
...
void Excitonic_Hamiltonian::print(vec excitonic_momentum,double eta,int tamn_dancoff,Coulomb_Potential* coulomb_potential,Dielectric_Function* dielectric_function,int adding_screening){
...
int main()
{
	...
	int number_conduction_bands_selected=1;
	int number_valence_bands_selected=1;
	...
	//dipole_elements.print(excitonic_momentum,excitonic_momentum,3,0);
	...
	int tamn_dancoff=1;
	Excitonic_Hamiltonian htbse(number_valence_bands_selected,number_conduction_bands_selected,k_points_list,number_k_points_list,g_points_list,number_g_points_list,spinorial_calculation,htb_basis_dimension,&dipole_elements);
	//htbse.print(excitonic_momentum,eta,tamn_dancoff,&coulomb_potential,&dielectric_function,adding_screening);
	///calculation optical spectrum
	string file_macroscopic_dielectric_function_bse_name="macroscopic_diel_func_bse.dat";
	htbse.pull_macroscopic_bse_dielectric_function(omegas_path,number_omegas_path,eta,file_macroscopic_dielectric_function_bse_name,lorentzian,tamn_dancoff,&coulomb_potential,&dielectric_function,1);
