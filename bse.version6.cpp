...
public:
	...
	{		
		...
		tuple<vec,mat> eigenvalues_and_eigenstates;
		eigenvalues_and_eigenstates=pull_eigenstates_through_cholesky_way(excitonic_momentum,epsilon,eta);
		vec eigenvalues=get<0>(eigenvalues_and_eigenstates);
		mat eigenstates=get<1>(eigenvalues_and_eigenstates);
		for (int i=0;i<spin_dimension_bse_hamiltonian;i++)
			printf("%2.6f\n",eigenvalues(i));
	};
	//cx_cube pull_excitonic_oscillator_force(cx_mat excitonic_eigenstates, double eta);
...
int main()
{
	...
	int number_conduction_bands_bse=2;
	int number_valence_bands_bse=2;
	Dipole_Elements dipole_elements;
